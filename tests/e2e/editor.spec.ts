...
test.describe("cutting the timeline", () => {
  ...
  test("the two halves still cover the timeline end to end", async ({ page }) => {
    await openEditor(page, { sources: [["red.png", RED]] });
    await waitForRender(page);
  ...
  test("splits again inside a segment that was already cut", async ({ page }) => {
    await openEditor(page, { sources: [["red.png", RED]] });
    await waitForRender(page);
...
test.describe("a segment's chain", () => {
  ...
  test("a mosh switches effects on and changes what's on screen", async ({ page }) => {
    await openEditor(page, { sources: [["red.png", RED]] });
