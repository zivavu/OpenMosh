...
/** Every effect, through the real renderer, on a real GPU context. A shader that stops
 * compiling is caught, logged and skipped, so nothing goes red; this file finds that. */
...
interface EffectResult {
	...
	/** Differed once every knob was turned up. An effect neutral at its defaults is
	 * normal (Zoom starts at 1), but one neutral here is dead. */
	changedTurnedUp: boolean;
	/** Flat black at every sampled moment, in both configurations. */
	black: boolean;
	glError: number;
	/** Frame hashes at the effect's default settings, in time order. Two effects sharing
	 * all of theirs are rendering the same thing. */
...
/** Rendered at 128px: big enough for spatial effects to displace, small enough to read
 * back 150 times under a software rasterizer. */
const SIZE = 128;

/** Several moments, none of them 0: one sample can't tell a broken effect from a periodic
 * one (Strobe is dark most of its cycle, and at t=0 every animated effect is a no-op). */
...
async function renderEveryEffect(page: Page): Promise<RenderReport> {
	...
	return page.evaluate(
		async ([size, times]) => {
			// A variable specifier: a literal would resolve at build time, not to the dev server URL.
			...
			// Detail in both axes, plus flat corners: an effect that shifts pixels needs an edge,
			// and one that shifts colour needs a gradient.
			...
			/** A loud, lopsided, *moving* spectrum. Moving matters: the renderer normalizes against
			 * a running per-bin floor, so a constant signal normalizes to zero and renders nothing. */
			...
			const sample = () => {
				// The chain leaves its own framebuffer bound; the canvas holds the finished frame.
			...
			/** The same effect with every knob pushed to the end of its range: ranges to max, switches
			 * on, a word in any text field. Separates "neutral until touched" from "wired to nothing". */
			const turnedUp = (def: {
				id: string;
				params: {
					key: string;
				...
				for (const param of def.params) {
					// Read by the host, not the shader, and "more" is the wrong direction: smoothing at
					// its max freezes the envelope follower at zero, reading as an effect that does nothing.
			...
			/** Second pass, for the effects the snapshots couldn't judge: a feedback or melt effect
			 * builds its picture from earlier frames, and a strobe is dark most of its cycle. */
			const FRAMES = 30;
			const STEP = 1 / 30;
			const START = 0.5;
			...
			for (const effect of effects) {
				...
				// Both configurations get played: turned up says whether the effect does anything,
				// and defaults decide blackness (Strobe with its duty maxed is meant to sit on black).
				const hot = ramp(turnedUp(def), def.id);
				const plain = ramp(
					hydrateValues(def.id, {}) as Record<string, number | string>,
					def.id,
				);
				// Against the whole passthrough ramp: a static chain renders the same frame every time.
...
/** One render pass per worker, shared by every test below. The pass costs a WebGL
 * context and ~150 software-rendered frames; per assertion that would be far slower. */
...
test("draws the source through an empty chain", async ({ report }) => {
	// If the baseline is black the whole report is meaningless: every effect reads as unchanged.
...
test("compiles every effect's shader", async ({ report }) => {
	// A compile or link failure is caught, logged, and then silent; this log is the only signal.
...
test("leaves no effect rendering flat black at every moment", async ({
	...
	// An unwritten framebuffer reads as black, not as an error, and looks like a strong effect.
...
test("every effect changes the frame once it's turned up", async ({
	...
	// The point of the whole file: an effect that renders cleanly and does nothing is
...
test("gives each effect its own look", async ({ report }) => {
	// Two effects landing on identical output at every sampled moment means one is wired
	// to the other's shader, a registry copy-paste nothing else would catch.
