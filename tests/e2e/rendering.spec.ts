...
async function renderEveryEffect(page: Page): Promise<RenderReport> {
  ...
  return page.evaluate(
    async ([size, times]) => {
      ...
      const { EFFECT_DEFINITIONS } = await load("/src/lib/effects/definitions.ts");
      ...
      const spectrumAt = (t: number) => {
        ...
        for (let i = 0; i < spectrum.length; i++) {
          ...
          spectrum[i] = Math.min(255, Math.round(255 * band * (0.15 + 0.85 * pulse)));
        }
        return spectrum;
      };
      ...
      const turnedUp = (def: {
        id: string;
        params: { key: string; type: string; max?: number; defaultValue: unknown }[];
      }) => {
        const values = hydrateValues(def.id, {}) as Record<string, number | string>;
      ...
      const instanceOf = (defId: string, values: Record<string, number | string>) => ({
      ...
      for (const def of EFFECT_DEFINITIONS) {
        ...
        const hot = times.map((t) => drawAt([instanceOf(def.id, turnedUp(def))], t));
        effects.push({
          id: def.id,
          name: def.name,
          changedAtDefaults: atDefaults.some((s, i) => s.hash !== passthrough[i].hash),
      ...
      const ramp = (values: Record<string, number | string> | null, defId: string) => {
      ...
      for (const effect of effects) {
        const undecided =
          (!effect.changedAtDefaults && !effect.changedTurnedUp) || effect.black;
        ...
        if (hot.hashes.some((h) => !baseline.has(h))) effect.changedTurnedUp = true;
        if (plain.hashes.some((h) => !baseline.has(h))) effect.changedAtDefaults = true;
...
test("leaves no effect rendering flat black at every moment", async ({ report }) => {
...
test("every effect changes the frame once it's turned up", async ({ report }) => {
