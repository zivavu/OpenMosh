...
test("a song worked on is offered back on the upload screen", async ({ page }) => {
  ...
  await page.goto("/");
  await page.getByRole("button", { name: "Editor", exact: true }).click();
  await expect(page.getByText("OR PICK UP A SONG YOU'VE WORKED ON")).toBeVisible();
  await expect(page.locator(".saved-item")).toHaveCount(1);
  await expect(page.locator(".saved-item")).toContainText("track.wav");
});

test("reopening it restores the cuts, the pool and every chain", async ({ page }) => {
  await openEditor(page, {
    sources: [
      ["red.png", RED],
      ["green.png", GREEN],
...
test("a sequence with nothing pooled is never offered back", async ({ page }) => {
