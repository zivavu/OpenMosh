...
export async function canvasStats(page: Page): Promise<CanvasStats> {
  return page.evaluate(
    async ([selector, sample]) => {
      const source = document.querySelector(selector) as HTMLCanvasElement | null;
...
export function nearestColor(mean: Rgb, palette: Record<string, Rgb>): string {
  ...
  for (const [name, color] of Object.entries(palette)) {
    const d =
      (mean.r - color.r) ** 2 + (mean.g - color.g) ** 2 + (mean.b - color.b) ** 2;
    if (d < bestDistance) {
      bestDistance = d;
      best = name;
    }
...
export async function splitSegmentAt(page: Page, fraction: number): Promise<void> {
