...
describe("hydrateValues", () => {
  ...
  it("passes values through untouched for an effect it doesn't know", () => {
    expect(hydrateValues("effect-deleted-three-versions-ago", { a: 1 })).toEqual({
      a: 1,
    });
    expect(hydrateValues("effect-deleted-three-versions-ago", undefined)).toEqual({});
  });
  ...
  describe("a range param", () => {
    ...
    it("clamps a value that fell outside a narrowed range", () => {
      expect(hydrateValues(def.id, { [param.key]: param.max + 1000 })[param.key]).toBe(
        param.max,
      );
      expect(hydrateValues(def.id, { [param.key]: param.min - 1000 })[param.key]).toBe(
        param.min,
      );
    });

    it("accepts a number that arrived as a string", () => {
      expect(hydrateValues(def.id, { [param.key]: String(param.min) })[param.key]).toBe(
        param.min,
      );
    });
  ...
  describe("a select param", () => {
    ...
    it("keeps an option that still exists", () => {
      ...
      expect(hydrateValues(def.id, { [param.key]: option })[param.key]).toBe(option);
    });

    it("falls back to the default when the option was removed", () => {
      expect(hydrateValues(def.id, { [param.key]: "mode-that-was-cut" })[param.key]).toBe(
        param.defaultValue,
      );
    });
  ...
  describe("a text param", () => {
    ...
    it("keeps any string, empty included", () => {
      ...
      expect(hydrateValues(def.id, { [param.key]: "hello" })[param.key]).toBe("hello");
    });
  ...
  describe("duotone's hues, saved before it took colors", () => {
    ...
    it("doesn't touch the same keys on another effect", () => {
      const hydrated = hydrateValues(EFFECT_DEFINITIONS[0].id, { shadowHue: 200 });
...
describe("hydrateEffects", () => {
  ...
  it("drops an instance whose effect no longer exists", () => {
    const chain = hydrateEffects([
      ...
      { defId: "effect-deleted-three-versions-ago", instanceId: "b", values: {} },
    ]);
  ...
  it("keeps the instance id it was saved with", () => {
    const chain = hydrateEffects([{ defId: known, instanceId: "keep-me", values: {} }]);
  ...
  it("hydrates the values of every survivor", () => {
    const def = EFFECT_DEFINITIONS[0];
    const chain = hydrateEffects([{ defId: def.id, instanceId: "a", values: {} }]);
    for (const param of def.params) {
  ...
  it("carries the rest of the instance across untouched", () => {
    const chain = hydrateEffects([
      { defId: known, instanceId: "a", enabled: false, locked: true, values: {} },
    ]);
    ...
    for (const def of EFFECT_DEFINITIONS) {
      for (const param of def.params) {
        ...
        for (const value of param.moshOptions) {
          if (!options.has(value)) wrong.push(`${def.id}.${param.key}:${value}`);
        }
      }
    }
    expect(wrong).toEqual([]);
