...
export function createBeatClock(
  ...
  const intervalSeconds =
    subdivision === 0 ? Infinity : (60 / bpm) * subdivision;
  return {
    intervalSeconds,
    beatAt(t: number) {
      if (subdivision === 0)
        return { index: Number.MAX_SAFE_INTEGER, fraction: 0 };
