...
describe("clipRange", () => {
	...
	it("reads the same range dragged backwards", () => {
		...
		const ids = lane.clips.map((c) => c.id);
		expect(clipRange(lane, ids[2], ids[0])).toEqual(
			clipRange(lane, ids[0], ids[2]),
		);
	});
...
describe("moveClips", () => {
	...
	it("stops at the track ends", () => {
		...
		const ids = lane.clips.map((c) => c.id);
		expect(moveClips(lane, ids, -10, 10).clips.map((c) => c.start)).toEqual([
			0, 2,
		]);
		expect(moveClips(lane, ids, 10, 10).clips.map((c) => c.end)).toEqual([
			8, 10,
		]);
	});
	...
	it("ignores ids from other lanes", () => {
		...
		expect(
			moveClips(lane, ["elsewhere"], 5, 10).clips.map((c) => c.start),
		).toEqual(before);
	});
