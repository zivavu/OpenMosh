...
function texture(rng: Rand, look: Look): Omit<GradientSpec, "gen" | "colors"> {
	return {
		...
		scale: look.scale,
		warp: randIn(rng, ...WARP_RANGE[look.mode]),
		angle: rng() * Math.PI * 2,
...
function planCohesive(
	...
	const baseScale = randomScale(rng, mode);
	return Array.from({ length: n }, (): GradientSpec => {
		...
		return {
			...
			...texture(rng, {
				mode,
				scale: baseScale * randLog(rng, 0.7, 1.4),
			}),
		};
	});
}
