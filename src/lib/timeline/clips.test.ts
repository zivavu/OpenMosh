...
describe("resizeClip", () => {
  it("drags either edge to the requested time", () => {
    expect(spans(resizeClip(laneOf(clip("a", 2, 6)), "a", "start", 3, 10))).toEqual(
      [["a", 3, 6]],
    );
    expect(spans(resizeClip(laneOf(clip("a", 2, 6)), "a", "end", 5, 10))).toEqual([
      ["a", 2, 5],
    ]);
  });
  ...
  it("clamps the trailing edge to the end of the timeline", () => {
    expect(spans(resizeClip(laneOf(clip("a", 0, 2)), "a", "end", 99, 10))).toEqual(
      [["a", 0, 10]],
    );
  });
...
describe("updateLaneIn", () => {
  ...
  it("leaves every lane alone when none matches", () => {
    ...
    expect(updateLaneIn(lanes, "missing", () => ({ id: "one", clips: [] }))[0]).toBe(
      lanes[0],
    );
  });
});
