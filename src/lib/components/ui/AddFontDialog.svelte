<script lang="ts">
	...
	interface Props {
		onClose: () => void;
		/** The family of the last face added, to select it. */
		onAdded: (family: string) => void;
		/** A face went away; the host drops it wherever it was picked. */
		onRemoved?: (family: string) => void;
	}

	let { onClose, onAdded, onRemoved }: Props = $props();
...
<div class="font-overlay" onclick={onClose} onkeydown={onKeydown}>
	<!-- svelte-ignore a11y_no_static_element_interactions -->
