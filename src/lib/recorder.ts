...
async function recordMp4WebM(opts: RecordOptions): Promise<Blob> {
  ...
  const { format, duration, fps, canvas, renderer, effects, onProgress, signal } = opts;
  ...
  const preferredCodecs = format === 'mp4'
  ...
  const selectedCodec = await mb.getFirstEncodableVideoCodec(candidates as any, {
    width: canvas.width,
