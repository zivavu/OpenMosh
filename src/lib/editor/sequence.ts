...
export type TransitionType =
	...
	| "crosszoom"
	| "cube"
	/** Picks one of the blends above per boundary, seeded. */
	| "random";
...
/** What one blend actually renders once any random picks are made. */
export interface ConcreteTransition {
	type: Exclude<TransitionType, "random">;
	direction: number;
	density: number;
}

/**
 * Concrete type/direction/density for one blend. Random picks are drawn from
 * the seed plus the boundary time, so on-tick transitions roll fresh each
 * tick while a given tick always lands the same way in preview and export.
 * A random type rolls its direction and density too — the stored ones belong
 * to whatever type was picked in the UI before.
 */
export function resolveConcreteTransition(
	...
	const density = transition.density ?? 1;
	return {
		type,
		direction:
			randomType || direction === RANDOM_DIRECTION
				? Math.floor(rng() * 4)
				: direction,
		density: randomType ? Math.floor(rng() * 3) : density,
	};
}
...
const LIVE_TRANSITIONS = new Set<string>(
 ...
 * a rhythm, so anything longer is left to the segment boundaries.
 */
...
export interface ResolvedTransition {
	...
	boundaryTime: number;
	transition: SegmentTransition;
	/** What this blend renders; `transition` itself may hold random picks. */
...
export function resolveTransitionAt(
	...
	if (seg.mode === "interval" && seg.transitionOnTick) {
		...
		if (tick > 0) {
			boundary =
				seg.startTime + tick * (seg.intervalSec ?? DEFAULT_INTERVAL_SEC);
		}
	}
	...
	return {
		...
		concrete: resolveConcreteTransition(seg.transition, boundary),
	};
}
